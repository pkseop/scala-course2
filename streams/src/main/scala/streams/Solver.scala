...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter(x => !explored.exists(e => e.b1 == x._1.b1 && e.b2 == x._1.b2))
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty)
      ...
      val more = for{
        b <- initial
        next <- newNeighborsOnly(neighborsWithHistory(b._1, b._2), explored)
      } yield next
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = {
    for{
      path <- pathsFromStart
      if(path._1.isStanding && path._1.b1.equals(goal))
    } yield path
  }
